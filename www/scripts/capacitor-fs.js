        ...
        reader.onloadend = async () => {
          ...
          try {
            ...
            if (perm.publicStorage === "denied") {
              await Toast.show({ text: "Permission denied to save file." });
              return;
            }
            const directory = localStorage.getItem("setting-gallery") === "true" && file.type && file.type.startsWith("image/") ? Directory.Pictures : Directory.Documents;
            await Filesystem.writeFile({ path: fileName, data: base64data, directory, recursive: true });
            await Toast.show({ text: "Saved " + fileName });
          } catch (e) {
